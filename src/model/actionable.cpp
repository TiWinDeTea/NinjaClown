...
void behaviours::gate(const instance_data &data, const argument_type &arg) noexcept {
	assert(data.pos.x >= 0 && static_cast<std::size_t>(data.pos.x) < arg.world.get_map().width()); // NOLINT
	assert(data.pos.y >= 0 && static_cast<std::size_t>(data.pos.y) < arg.world.get_map().height()); // NOLINT

	switch (arg.world.get_map()[data.pos.x][data.pos.y].type) {
		case cell_type::CHASM:
		case cell_type::WALL:
			utils::log::info("actionable.gate.open", "x"_a = data.pos.x, "y"_a = data.pos.y);
			arg.world.get_map()[data.pos.x][data.pos.y].type = cell_type::GROUND;
		...
		case cell_type::GROUND:
			...
			arg.world.get_map()[data.pos.x][data.pos.y].type = cell_type::WALL;
